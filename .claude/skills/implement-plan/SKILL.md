...
description: >-
  ...
  (extraction, dbt models, infrastructure, a new skill). Stage 4 builds *what stage 3 planned* — so do
...
`uv run pytest`, `uv run dbt build --target ci`, or the extractor against a fixture, and read the real
...
2. **Self-verify** the phase's own acceptance criteria *by running them* — `uv run pytest`,
   `uv run dbt build --target ci`, the extractor against a fixture. Read the **actual output**; do not
...
- **Resolve by name.** In dbt, `ref()` and `source()` — never a literal table or path. In Python,
  through the config layer — never a hardcoded path or a `parents[N]` walk.
- **The landing zone is immutable.** Raw payloads are written once. Code that mutates or overwrites a
  landed file is a blocker, not a style note — it destroys the ability to diagnose incidents by diffing.
- **Bronze stays 1:1 with the source**; **silver declares its grain and proves it** with a uniqueness
  test; **facts MERGE on key** rather than appending, because box scores get restated. A silver model
  whose declared grain has no test is not green.
- **Era boundaries are explicit.** Tracking-derived columns don't exist before 2013-14. A model that
  averages across that boundary without handling it is producing wrong numbers, not incomplete ones.
- **Anything that spends cloud money or touches prod is user-run.** Stage it as a script and route it to
