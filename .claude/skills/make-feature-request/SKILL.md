...
description: >-
  ...
  new dbt model or layer, a new skill, new infrastructure, a new orchestration DAG, a serving
  ...
  feature-request directory, or basketball questions the existing marts already answer — those
...
- a **bug fix**, a **data incident**, or an **analysis question** the existing marts already
...
- an idea that **plainly collides with a hard constraint**. The common ones here: the data
  simply isn't publicly available (raw tracking, wearables, medical — see
  [`docs/data-sources.md`](../../../docs/data-sources.md)), or the requested history predates the
  source's coverage (tracking-derived stats do not exist before 2013-14; this project starts at
  2003-04). Don't silently write a request for a dead-on-arrival idea: name the specific
...
- Which subsystem? Extraction/landing (`src/nba_platform/`), transformation (`transform/models/`
  — and which medallion layer), orchestration (`orchestration/`), infrastructure (`infra/`),
  serving (`app/`), a skill (`.claude/skills/`), docs (`docs/`)?
- What already exists that this overlaps, extends, or must join to? Name the dbt models by
...
If basketball specifics matter to the request (what a stat means, how a rule change affects
...
<Which part of the repo this touches, and the concrete files / dbt models (by ref name) /
