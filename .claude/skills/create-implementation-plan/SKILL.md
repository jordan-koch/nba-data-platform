...
description: >-
  ...
  write the feature's code, or for basketball analysis questions the existing marts already answer.
...
plus `uv run dbt build --target ci` when models changed) → `/commit`, which stages, checks the docs,
...
<the CLAUDE.md rules the implementer must honor: resolve by name (dbt ref()/source(), never a literal
path); the landing zone is immutable; bronze is 1:1 with the source; silver declares its grain and
proves it with a uniqueness test; facts MERGE on key rather than append; commits go through /commit only;
subagents get read-only git. Add the cost rule only when the change can spend cloud money. Don't force
irrelevant conventions onto a change that doesn't touch data.>
...
- **Every relative link and bare `requests/...` token you write must resolve on disk.** A live
  (non-`_done/`) artifact body is scanned by `tests/test_doc_links.py` along with every other
  Markdown file, a blocking CI check, so a dead pointer here fails the build rather than quietly
  misleading the next stage. Two
