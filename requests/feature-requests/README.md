...
| [agent-dispatch](agent-dispatch/) | scoped | **The feature IS a pipeline stage** — modifies `/implement-plan` (stage 4) so it routes a plan's target paths to a builder instead of always building in-thread. The sequel to `data-engineer-agent`. Fit **reshape**, disposed proceed: the recorded allowlist-subset design is refuted by `box-score-foundation`'s own worked example, so routing is on the deny fence **plus** declared actions, per phase, with split as the normal outcome. |
